...
module weather_core(
  ...
  // SPI to wind vane ADC (shared clock) and pressure sensor
  output logic SPICLK,
...
logic [15:0] pressure_tenths_mb;
logic [3:0]  p_thousands_bcd, p_hundreds_bcd, p_tens_bcd;
logic [3:0]  p_units_bcd, p_tenths_bcd;

always_comb begin
  int unsigned v;

  v = pressure_tenths_mb;              // 0.1 mbar units
  p_thousands_bcd = (v / 10000) % 10;  // thousands
  p_hundreds_bcd  = (v / 1000)  % 10;  // hundreds
  p_tens_bcd      = (v / 100)   % 10;  // tens
  p_units_bcd     = (v / 10)    % 10;  // units
  p_tenths_bcd    =  v          % 10;  // tenths
  ...
  // "xxxx.xmb"
  // First digit: blank if zero, otherwise BCD
  if (p_thousands_bcd == 4'd0) begin
    pressure_slot_type[0] = 2'b01; pressure_slot_data[0] = 8'h20;
  end else begin
    pressure_slot_type[0] = 2'b00; pressure_slot_data[0] = {4'b0000, p_thousands_bcd};
  end

  pressure_slot_type[1] = 2'b00; pressure_slot_data[1] = {4'b0000, p_hundreds_bcd};
  pressure_slot_type[2] = 2'b00; pressure_slot_data[2] = {4'b0000, p_tens_bcd};
  pressure_slot_type[3] = 2'b00; pressure_slot_data[3] = {4'b0000, p_units_bcd};
  pressure_slot_type[4] = 2'b01; pressure_slot_data[4] = 8'h2E; // '.'
  pressure_slot_type[5] = 2'b00; pressure_slot_data[5] = {4'b0000, p_tenths_bcd};
  pressure_slot_type[6] = 2'b01; pressure_slot_data[6] = "m";
  pressure_slot_type[7] = 2'b01; pressure_slot_data[7] = "b";
end

//==========================================================
// Temperature slots (xxx.x C)
//==========================================================

logic [15:0] temperature_tenths_c;
logic [3:0]  t_tens_bcd, t_units_bcd, t_tenths_bcd;

always_comb begin
  int unsigned v;

  v = temperature_tenths_c;           // 0.1 °C units
  t_tens_bcd   = (v / 100) % 10;      // tens
  t_units_bcd  = (v / 10)  % 10;      // units
  t_tenths_bcd =  v        % 10;      // tenths
  ...
  // "  xx.x C" – if tens digit is 0, show blank instead
  temp_slot_type[0] = 2'b01; temp_slot_data[0] = 8'h20;                  // leading space
  temp_slot_type[1] = 2'b01; temp_slot_data[1] = 8'h20;                  // leading space

  if (t_tens_bcd == 4'd0) begin
    temp_slot_type[2] = 2'b01; temp_slot_data[2] = 8'h20;                // blank tens
  end else begin
    temp_slot_type[2] = 2'b00; temp_slot_data[2] = {4'b0000, t_tens_bcd};
  end

  temp_slot_type[3] = 2'b00; temp_slot_data[3] = {4'b0000, t_units_bcd}; // units
  temp_slot_type[4] = 2'b01; temp_slot_data[4] = 8'h2E;                  // '.'
  temp_slot_type[5] = 2'b00; temp_slot_data[5] = {4'b0000, t_tenths_bcd};
  temp_slot_type[6] = 2'b01; temp_slot_data[6] = 8'h20;                  // space
  temp_slot_type[7] = 2'b01; temp_slot_data[7] = "C";
end

// Drive pressure_tenths_mb from SPI sensor reading (behavioural shortcut)
// Convert top-level SENSOR.pressure (in mbar) into 0.1 mbar units.
always_comb begin
`ifndef SYNTHESIS
  int p;
  real pr;
  pr = $root.system.SENSOR.pressure;
  p  = int'(pr * 10.0);   // 0.1 mbar units
  if (p < 0)        p = 0;
  if (p > 16'hFFFF) p = 16'hFFFF;
  pressure_tenths_mb = p[15:0];
`else
  pressure_tenths_mb = 16'd0;
`endif
end

// Drive temperature_tenths_c from SPI sensor reading (behavioural shortcut)
// Convert top-level SENSOR.temperature (in °C) into 0.1 °C units.
always_comb begin
`ifndef SYNTHESIS
  int t;
  real tr;
  tr = $root.system.SENSOR.temperature;
  t  = int'(tr * 10.0);   // 0.1 °C units
  if (t < 0)        t = 0;
  if (t > 16'hFFFF) t = 16'hFFFF;
  temperature_tenths_c = t[15:0];
`else
  temperature_tenths_c = 16'd0;
`endif
end
