...
module anemometer #(
    ...
    // ----------------------------------------------------------
    // 1) Edge detect
    //    anemo_sw is already synchronised in weather.sv (CORE_nWind),
    //    so we only need a single previous-cycle register here.
    // ----------------------------------------------------------
    logic prev_sw;
    always_ff @( posedge Clock, negedge nReset)
        if(!nReset)
            prev_sw <= 1'b1;       // active-low input idles high
        else
            prev_sw <= anemo_sw;


    // nWind/anemo switch is active-low: count falling edges (high -> low)
    wire sw_fall_raw = ~anemo_sw & prev_sw;
