...
module weather_core(
  ...
  // Scan path (always present; Test & ScanEnable select shift mode — matches gate-level DFT)
  output logic SDO,
  input  logic Test,
  input  logic SDI,
...
//----------------------------------------------------------------------
// Scan path (behavioural chain; SDO never floats in functional mode)
//----------------------------------------------------------------------
wire scan_shift = Test & ScanEnable;
...
always_ff @(posedge Clock or negedge nReset) begin
  if (!nReset)
    ...
    scan_sr <= {scan_sr[6:0], SDI};
end

// Functional mode: SDO driven low. Scan shift: MSB of internal chain.
assign SDO = scan_shift ? scan_sr[7] : 1'b0;
